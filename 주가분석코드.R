2020년 유가증권시장 상장 기업의 성과 분석을 통해 투자 특성을 파악해 최적의 투자 전략을 도출
설명변수: 시가총액, 주가, 배당수익률, PER, PBR, 분기
종속변수: 로그수익률, 일중변동성, 역사적변동성, 회전율, 거래량

*데이터 전처리
한국거래소(KRX)에 상장된 종목들의 데이터를 Python을 이용하여 크롤링 (pykrx패키지 사용)
1939개 종목 중 PER, PBR과 같은 펀더멘탈을 산출할 수 없는 종목과 결측치를 제외한 총 1827개종목 선택

*결론
시가총액과 주가가 클수록 변동성 낮으며, 배당수익률이 높은 종목은 안정성을 보였으나 수익률은 저조한 경향
PER과 PBR이 높은 그룹의 수익률과 유동성이 변동성 모두 크게 나타나 공격적 투자 성향을 보였다.
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
rm(list=ls())
